...
export function handleCreateBin(action: Extract<CommandAction, { type: 'create_bin' }>, ctx: ActionContext): ActionResult {
  ...
  if (action.custom_fields && typeof action.custom_fields === 'object') {
    replaceCustomFieldValuesSync(binId, action.custom_fields as Record<string, string>, ctx.locationId);
  }
...
export function handleDuplicateBin(action: Extract<CommandAction, { type: 'duplicate_bin' }>, ctx: ActionContext): ActionResult {
