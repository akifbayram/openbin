...
You join with the location's default role (typically member). Admins can change your role after joining.
...
Admins can change any member's role from **Location Settings → Members**.
...
The `REGISTRATION_MODE` environment variable controls who can create an account:

| Mode | Behavior |
|---|---|
| `open` | Anyone can register (default) |
| `invite` | New users must enter a location invite code during sign-up |
| `closed` | No new registrations; existing users can still log in |

When set to `invite`, the registration page asks for an invite code. The user is automatically added to the corresponding location after registration.
