...
The `REGISTRATION_MODE` environment variable controls who can create an account (`open`, `invite`, or `closed`). See [Configuration](/getting-started/configuration) for details. When set to `invite`, new users enter an invite code during sign-up and are automatically added to the corresponding location.
