...
- **API response envelopes**: Lists return `{ results: T[], count }`. Errors return `{ error: "CODE", message }` with codes: `UNAUTHORIZED` (401), `VALIDATION_ERROR` (422), `NOT_FOUND` (404), `FORBIDDEN` (403), `CONFLICT` (409), `INTERNAL_ERROR` (500).
