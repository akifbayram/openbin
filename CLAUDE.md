...
- **AI**: `GET /api/ai/settings`, `PUT /api/ai/settings`, `DELETE /api/ai/settings`, `POST /api/ai/analyze` (by photoId), `POST /api/ai/analyze-image` (raw file upload via FormData), `POST /api/ai/structure-text` (dictated text → structured items), `POST /api/ai/command` (natural language → structured actions), `POST /api/ai/test`
...
- **Voice dictation / text structuring**: `DictationInput` component in `features/ai/` lets users dictate or type freeform text, sends it to `POST /api/ai/structure-text`, and shows a preview checklist before adding items. `useTextStructuring()` hook provides `{ structuredItems, isStructuring, error, structure, clearStructured }`. `structureTextItems(options)` is the plain async function. `ItemsInput` accepts optional `showDictation`, `aiConfigured`, `onAiSetupNeeded`, `binName`, `locationId` props to render a Mic button that toggles the `DictationInput`. Server-side `structureText()` in `server/src/lib/structureText.ts` handles LLM calls with a dedicated prompt for item extraction.
- **AI commands / natural language**: `CommandInput` dialog in `features/ai/` lets users type freeform commands like "add screwdriver to the tools bin" or "move batteries from kitchen to garage". `POST /api/ai/command` parses the command via the user's AI provider into structured `CommandAction[]` (discriminated union on `type`: `add_items`, `remove_items`, `modify_item`, `create_bin`, `delete_bin`, `add_tags`, `remove_tags`, `modify_tag`, `set_area`, `set_notes`, `set_icon`, `set_color`). The client previews actions with checkboxes, then executes them using existing mutation functions (`addBin`, `updateBin`, `deleteBin`, `createArea`). `useCommand()` hook provides `{ actions, interpretation, isParsing, error, parse, clearCommand }`. `parseCommandText(options)` is the plain async function. Server-side `parseCommand()` in `server/src/lib/commandParser.ts` handles LLM calls. Entry points: Dashboard and BinListPage add menus (lazy-loaded). Context payload includes all bins and areas for fuzzy matching. Custom command prompt stored in `user_ai_settings.command_prompt` — customizable via Settings alongside the image analysis prompt. `DEFAULT_COMMAND_PROMPT` in `defaultCommandPrompt.ts` (client) and `commandParser.ts` (server).
