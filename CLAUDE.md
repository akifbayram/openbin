...
- **Snake_case field names** on DB-backed interfaces (`Bin`, `Photo`, `Location`, `LocationMember`) to match PostgreSQL columns. Export types remain camelCase.
...
- **Lazy loading**: Dashboard, Scanner, Print, Settings, Auth, Locations, Profile, Tags, and Items pages are `React.lazy` with `<Suspense>`.
- **`cn()` helper** (clsx + tailwind-merge) for conditional class composition.
- **`addBin()` accepts an options object** (`AddBinOptions`): `{ name, locationId, items?, notes?, tags?, location?, icon?, color? }`. `locationId` is required. `short_code` is auto-generated by the server.
...
- **useBins.ts exports**: hooks (`useBinList`, `useBin`, `useAllTags`), plain functions (`addBin`, `updateBin`, `deleteBin`, `restoreBin`, `lookupBinByCode`, `notifyBinsChanged`), types/constants (`SortOption`, `BinFilters`, `EMPTY_FILTERS`, `countActiveFilters`).
