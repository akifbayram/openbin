...
export function Switch({ checked, onCheckedChange, id, disabled, className }: SwitchProps) {
  return (
    ...
    >
      <span
        className={`pointer-events-none inline-block h-[24px] w-[24px] rounded-full bg-white shadow-sm transition-transform duration-200 mt-[2px] ${checked ? 'translate-x-[24px]' : 'translate-x-[2px]'}`}
      />
    </button>
  );
}
