...
export function DashboardRecentScans({ bins, scanTimeMap, limit = 3, isLoading, showTimestamps = true }: DashboardRecentScansProps) {
    ...
    <section aria-labelledby="dash-recent-scans" className="flex flex-col gap-2">
      ...
      <div className={cn(flatCard, 'p-2')}>
        {isLoading && rows.length === 0 ? (
          <RecentScansSkeleton count={limit} />
        ) : (
...
function RecentScansSkeleton({ count }: { count: number }) {
  return (
    <ul className="flex flex-col">
      {Array.from({ length: count }).map((_, i) => (
        // biome-ignore lint/suspicious/noArrayIndexKey: static skeleton
        <li key={i} className="flex items-center gap-3 px-2.5 py-2 min-h-[52px]">
