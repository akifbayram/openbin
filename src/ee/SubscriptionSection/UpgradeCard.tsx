...
export function UpgradeCard({ targetPlan, action }: UpgradeCardProps) {
  ...
  return (
    <div className="rounded-[var(--radius-lg)] border border-[var(--border-flat)] p-5 text-center space-y-3">
      <h3 className="text-base font-semibold text-[var(--text-primary)]">Get {targetPlan.name}</h3>
      <p className="text-sm text-[var(--text-tertiary)]">
        {monthly}/month{annual && ` · ${annual}/year`}
      </p>
